        ...
        for i in 0..max_size.1 as i32 {
            for j in 0..max_size.0 as i32 {
                    ...
                    && y + cam_offset.1 < self.map.size as i32
                {
                    ...
                    {
                        dst_rect.x = x * dst_rect.w / 2 - y * dst_rect.h / 2 - dst_rect.w*2 - self.camera.position.x as i32 % dst_rect.w;
                        dst_rect.y = y * dst_rect.h / 4 + x * dst_rect.w / 4 - dst_rect.h/2 - self.camera.position.y as i32 % dst_rect.h;
                        self.canvas
